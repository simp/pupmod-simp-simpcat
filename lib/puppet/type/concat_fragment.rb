Puppet::Type.newtype(:concat_fragment) do
  ...
  newproperty(:content) do
    defaultto('!!concat_fragment_undef_content')
    ...
    def insync?(is)
      ...
      file = "#{Puppet[:vardir]}/concat/fragments/#{resource[:group]}/#{resource[:fragment]}"
      return false unless File.exist?(file)

      if resource[:content] == '!!concat_fragment_undef_content' then
        true
      else
  ...
  autorequire(:file) do
    if catalog.resources.find_all { |r| r.is_a?(Puppet::Type.type(:concat_build)) and r[:name] == self[:group] }.empty? then
      err "No 'concat_build' specified for group #{self[:group]}!"
    end
    ""
  end

  validate do
    if self[:externally_managed] == :false and self[:content] == '!!concat_fragment_undef_content' then
